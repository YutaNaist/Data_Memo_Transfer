...
class Dialog_Ask_ID(QtWidgets.QDialog):
    ...
    def log_In_To_Diamond(self):
        try:
            ...
            if str_Experiment_ID is not None:
                ...
                if self.data_Model.get_Dict_Data_Model("is_exist_temp_file") is False:
                ...
                self.data_Model.set_Dict_Data_Model(
                    "str_experiment_id", str_Experiment_ID
                )
                self.data_Model.set_Dict_Data_Model(
                    "dict_user_information", self.userData
                )
                ...
                if self.userData["data_delivery"]["is_share_with_google"] == True:
                    self.data_Model.set_Dict_Data_Model("is_share_with_google", True)

                self.dialog_Set_Initial = Dialog_Set_Initial(data_Model=self.data_Model)
    ...
    def check_ID_Request_To_Diamond(self, experiment_ID: str) -> str:
        ...
        if experiment_ID != "":
            self.data_Model.write_to_logger("Ask experiment ID to diamond")
            ...
            if reply["status"] is True:
                if (
                    ...
                    and self.data_Model.get_Dict_Data_Model("str_experiment_id") != ""
                ):
                    self.userData = reply["args"]["database"]
                    strSetText = ""
                    strSetText += "Warning!\n"
                    strSetText += (
                        "Your Experiment ID : " + str(self.userData["id"]) + "\n"
                    )
                    ...
                    strSetText += "Experiment ID : " + str(self.userData["id"]) + "\n"
                    strSetText += (
                        "User Name : "
                        + str(self.userData["creators"][0]["name"])
                        + "\n"
                    )
                    strSetText += (
                        "Instrument : "
                        + str(self.userData["instrument"]["name"])
                        + "\n"
                    )
                    strSetText += (
                        "Start Date : "
                        + str(self.userData["experiment_date"]["start_date"])
                        + "\n"
                    )
                    strSetText += "Are you sure to start experiment?"
                ...
                strSetText += "Experiment ID : " + str(self.userData["id"]) + "\n"
                strSetText += (
                    "User Name : " + str(self.userData["creators"][0]["name"]) + "\n"
                )
